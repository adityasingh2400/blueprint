...
export function IconsNext() {
    ...
    const iconCards = filteredIcons.map(icon => <IconCard icon={icon} key={icon.iconName} />);
...
function IconCard({ icon }: { icon: NextIconManifestEntry }) {
    const { iconName, hasFilled } = icon;
    const outlinedExportName = `${kebabToPascal(iconName)}Icon`;
    const filledExportName = `${kebabToPascal(iconName)}FilledIcon`;
    ...
    return (
        <div className="docs-icon-container">
            <div className="docs-icon">
                ...
                <div className="docs-icon-name">{iconName}</div>
...
function filterIcon(filter: string, icon: NextIconManifestEntry) {
    ...
    return smartSearch(filter, icon.iconName, icon.tags);
}
