...
export class IconsNext {
    ...
    private static filledSet: Set<string> = new Set(nextIconManifest.filter(e => e.hasFilled).map(e => e.iconName));

    /** Set of all valid next icon names, derived from the manifest. */
    private static validSet: Set<string> = new Set(nextIconManifest.map(e => e.iconName));
     ...
     */
    public static async load(icons: IconName | IconName[], variant: NextIconVariant = "outlined") {
    ...
    public static async loadAll(options?: { includeFilled?: boolean }) {
        const allNames = nextIconManifest.map(e => e.iconName) as IconName[];
        await this.load(allNames, "outlined");
        if (options?.includeFilled) {
            const filledNames = nextIconManifest.filter(e => e.hasFilled).map(e => e.iconName) as IconName[];
     ...
     */
    public static getPaths(icon: IconName, variant: NextIconVariant = "outlined"): IconPaths | undefined {
     ...
     */
    public static hasFilled(icon: IconName): boolean {
     ...
     */
    public static isValidIconName(name: string): name is IconName {
        return this.validSet.has(name);
    }

    private static async loadImpl(icon: IconName, variant: NextIconVariant) {
    ...
    private static resolveVariant(icon: IconName, variant: NextIconVariant): NextIconVariant {
        return variant === "filled" && this.filledSet.has(icon) ? "filled" : "outlined";
    }

    private static cacheKey(icon: IconName, variant: NextIconVariant): string {
        return `${icon}:${variant}`;
    }
}
