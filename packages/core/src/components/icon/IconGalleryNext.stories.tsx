...
const OUTLINED_ICONS = [...nextIconManifest].sort((a, b) => a.iconName.localeCompare(b.iconName));
const FILLED_ICONS = nextIconManifest.filter(e => e.hasFilled).sort((a, b) => a.iconName.localeCompare(b.iconName));
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {icons.map(entry => (
                    <Card key={entry.iconName} title={entry.iconName} style={ICON_GRID_CARD_STYLE}>
                        {renderNextIcon(entry.iconName, size, variant)}
                    </Card>
                ))}
            </div>
        </div>
...
function CompareGallery({ icons, size }: { icons: NextIconManifestEntry[]; size: number }) {
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {icons.map(entry => (
                    <CompareCard key={entry.iconName} iconName={entry.iconName} size={size} />
                ))}
            </div>
        </div>
    );
